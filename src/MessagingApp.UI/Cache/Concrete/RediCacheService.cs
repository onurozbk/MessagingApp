...
namespace MessagingApp.UI.Cache.Concrete
{
    public class RedisCacheService : ICacheService
    {
        ...
        {
            return await _cache.StringSetAsync(key, JsonSerializer.SerializeToUtf8Bytes(value), ExpireTime);
        }

        public T GetOrAdd<T>(string key, Func<T> action) where T : class
        {
            var result = _cache.StringGet(key);
            if (result.IsNull || result.ToString() == "null" || result.ToString() == "[]" || result.ToString() == "{}")
            {
